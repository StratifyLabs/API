 ...
 *
 * //because open() has variable arguments we wrap it to make it work with this
 * class int test_open(const char * path, int o_flags, int o_mode){ return
 * open(path, o_flags, o_mode);
 * }
 *
 ...
 *   Function<int, const char *, int, int> open_function_test("open",
 * test_open);
 *
 *   open_function_test.execute("NO EXIST Read only", -1, ENOENT,
 * "/home/non-existing-file.txt", O_RDONLY, 0); open_function_test.execute("NO
 * EXIST Read Write", -1, ENOENT, "/home/non-existing-file.txt", O_RDWR, 0);
 *   open_function_test.execute("NO EXIST Read Write", -1, ENAMETOOLONG,
 * "/home/non-existing-file-too-long-too-long-too-long-too-long-too-long-too-long-too-long-too-long-too-long-too-long.txt",
 * O_RDWR, 0);
 *
 *   //open_function_test will call the deconstructor here before
 * Test::finalize()
 * }
 *
 *
...
public:
  ...
  using ExpectedReturn =
      arg::Argument<return_type, struct FunctionExpectedErrorTag>;
   ...
   *
   */
  Function(const var::String &test_name, return_type (*function)(args...),
           Test *parent = 0)
   ...
   *
   */
  return_type execute_case_with_expected_return(const char *case_name,
    ...
    m_case_name.erase(var::String::Position(m_case_name.length() - 1));
    m_case_name << ")";

    {
      ...
      if ((return_value >= 0) && return_value != expected_value) {
        result = false;
        print_case_message("expected return %d != actual return %d",
      ...
      if (expected_errno > 0) {
        if (expected_errno != errno) {
          result = false;
          print_case_message("expected errno %d != actual errno %d",
  ...
  return_type expect_result(return_type expected_result, args... arguments) {
    ...
    errno = 0;
    {
      ...
      if (return_result != expected_result) {
        print_case_failed("result is not equal to expected result");
      }
    }
    return return_result;
  }

  return_type execute_case_with_less_than_zero_on_error(const char *case_name,
                                                        int expected_errno,
                                                        args... arguments) {
    return_type return_value;
    bool result = true;
...

    {
      ...
      if (expected_errno > 0) {
        ...
        if (return_value >= 0) {
          print_case_message("expected an error but returned: %d",
                             return_value);
          result = false;
        }
        ...
        if (return_value < 0) {
          result = false;
          print_case_message("expected success but ret: %d errno: %d",
                             return_value, errno);
        } else {
  ...
  return_type expect_error(int expected_errno, args... arguments) {
    return_type return_value;
    bool result = true;
    errno = 0;
    ...
    errno = 0;
    {
      ...
      if (errno != expected_errno) {
        print_case_failed("expected errno %d != actual errno %d",
                          expected_errno, errno);
      }
    }
    return return_value;
